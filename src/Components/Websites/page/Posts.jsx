...
const Posts = ({ ads, selectedCategory, selectedBrand, sponsorImages, loading, loadingMore }) => {
    ...
    // أضف راعي (sponsor) بعد كل 5 إعلانات
    ...
    let realAdCount = 0; // عداد الإعلانات الحقيقية فقط
    ...
    return (
        <div className='flex flex-col gap-5'>
            ...
            ) : adsWithSponsor.length > 0 ? (
                adsWithSponsor.map((item, index) => {
                    if (item === 'sponsor') {
                        return (
                            <motion.div
                                key={`sponsor-${index}`}
                        ...
                        const delayIndex = realAdCount % 10;
                        realAdCount++; // زيادة العداد فقط عند الإعلانات الحقيقية
                                    ...
                                    <div className='flex gap-4 lg:gap-10'>
                                        ...
                                        <div className='w-1/2 lg:w-auto truncate'>
                                            <div className='flex gap-1'>
                                                ...
                                                <h1 className='text-[10px] lg:text-[14px] text-placeholder'>{item.priceUSD.toLocaleString()}</h1>
                                            </div>
                                            <div className='flex gap-1'>
                                                <img className='w-4 md:w-5' src={pricesy} alt="" />
                                                <h1 className='text-[10px] lg:text-[14px] text-placeholder'>{item.priceSYP.toLocaleString()}</h1>
                                            </div>
                                        </div>
                                    </div>
