...
const AddPostEducation = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!title || !selectedLocation || !selectedBrand || images.length === 0) {
            alert('الرجاء تعبئة جميع الحقول المطلوبة (العنوان، الموقع، الصور، النوع...).');
            return;
        }
        ...
        try {
            ...
            console.log('تم إضافة الإعلان بنجاح:', response.data);
            alert('تم إضافة الإعلان بنجاح!');
        } catch (error) {
            console.error('حدث خطأ أثناء إضافة الإعلان:', error.response?.data || error.message);
