...
const CommentBox = ({ adId }) => {
    ...
    return (
        <div className="w-full mx-auto p-4">
            <div className="mt-6">
                ...
                {loading ?
                    (
                        ...
                        <p className="text-[12px] text-border">لا توجد تعليقات حتى الآن.</p>
                    ) : (
                            ...
                            <li key={index} className="bg-[#f7f7f7d1] rounded-10px mb-3 list-none">
                                <div className="p-[16px] leading-5 border-b-2">
                                    <div className="flex flex-col">
                                        <span className="text-[12px]">{c.userId.username || "مستخدم"}</span>
                                        <span className="text-[12px]">{dayjs(c.createdAt).fromNow()}</span>
                                    </div>
                                </div>

                                <div className="p-[16px] flex flex-col gap-3">
                                    <span className="text-[18px]">{c.content}</span>

                                    <div
                                        className="border-2 px-6 py-2 rounded-[20px] w-fit text-[16px] flex items-center gap-1 text-border cursor-pointer"
                                    ...
                                    {replies[index] && (
                                        <ul className="mt-2 pl-4 space-y-1 border-l border-gray-300">
                                            {replies[index].map((reply, i) => (
                                                <li key={i} className="text-sm text-gray-700">{reply}</li>
                                            ))}
                                        </ul>
                                    )}
                                </div>
            ...
            <h2 className="text-xl font-semibold my-4">أضف تعليقًا</h2>

            <form onSubmit={handleSubmit} className="flex flex-col space-y-3">
                <textarea
                    className="border p-[16px] h-[182px] rounded-10px resize-none text-[16px] focus:outline-none focus:border-primary"
                ...
                <button
                    type="submit"
                    className="bg-primary rounded-10px w-fit text-white text-[16px] py-2 px-8 flex items-center gap-1"
                >
                    إرسال
                    <BsFillSendFill />
                </button>
